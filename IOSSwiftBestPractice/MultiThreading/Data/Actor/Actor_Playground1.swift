...
// imagine at Bank, we have a model named `BankAccount`, and `BankAccount` has fields named `accountName`, and `balance`.
// The user can deposit money
// The user can transfer money from one account to another account

final class Actor_Playground1 {
    ...
    func run() {
        ...
        concurentQueue1.async { [weak self] in
            ...
            self.bankAccountOne.transfer(amount: 50, to: &self.bankAccountTwo, onThread: 1)
        }
        ...
        concurentQueue2.async { [weak self] in
            guard let self = self else { return }
            self.bankAccountOne.transfer(amount: 70, to: &self.bankAccountTwo, onThread: 1)
        }
    }
}
